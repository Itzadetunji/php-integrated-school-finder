...
<?php
	...
	function register_as_employee(){
		...
		if ($insert_user_code) {
			header("location:../sign-up?r=Parent has been successfully registered&ty=success");
		} else {
	...
	function register_as_employer(){
		...
		$insert_user_code = mysqli_query($connection, $insert_user);
		if ($insert_user_code) {
			header("location:../sign-up?r=School has been successfully registered&ty=success");
		} else {
			header("location:../sign-up?r=Sorry Error Processing Request. Try Again&ty=danger");
		}
	}
